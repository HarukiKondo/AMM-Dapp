...
contract AMM {
  ...
  /**
   * プールで指定されていないトークンでないか確認する修飾子
   */
  modifier validToken(IERC20 _token) {
    require(_token == tokenX || _token == tokenY, "Token is not in the pool");
    _;
  }

  modifier validTokens(IERC20 _tokenX, IERC20 _tokenY) {
    require(_tokenX == tokenX || _tokenY == tokenY, "Token is not in the pool");
    require(_tokenY == tokenX || _tokenY == tokenY, "Token is not in the pool");
  ...
  function getEquivalentToken(
    IERC20 _inToken,
    uint256 _amountIn
  ) public view activePool validToken(_inToken) returns (uint256) {
  ...
  function provide(
    ...
    if (totalShare == 0) {
      ...
      require(shareX == shareY, "Equivalent value of tokens not provided...");
      newshare = shareX;
    }

    require(newshare > 0, "Asset value less than threshold for contribution!");
  ...
  function getWithdrawEstimate(
    IERC20 _token,
    uint256 _share
  ) public view activePool validToken(_token) returns (uint256) {
  ...
  /**
   * Swap元のトークン量からSwap先のトークン量を算出するメソッド
   */
  ...
  function getSwapEstimateIn(
    IERC20 _outToken,
    uint256 _amountOut
  ) public view activePool validToken(_outToken) returns (uint256) {
    require(_amountOut < totalAmount[_outToken], "Insufficient pool balance");
